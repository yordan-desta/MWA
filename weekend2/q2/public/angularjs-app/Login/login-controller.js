angular.module('nobelPrizeApp').controller('LoginController', LoginController);

function LoginController(UserFactory, $window, AuthFactory, jwtHelper, $location) {
    ...
    vm.login = function() {
        ...
        UserFactory.login(vm.credentials).then(function(result) {
            ...
            if (result.status == 200) {
                ...
                const decoded = jwtHelper.decodeToken(result.data.token);

                vm.loggedinUser = decoded.username;
                $location.path('/');
