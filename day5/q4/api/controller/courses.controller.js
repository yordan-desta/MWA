...
module.exports.createCource = function(req, res) {
    ...
    Student.findById(req.params.courseId).select('courses').exec(function(err, doc) {
        const response = {
            status: 200,
        }
        if (err) {
...
module.exports.performFullUpdate = function(req, res) {
    ...
    Student.findById(req.params.studentId).select("courses").exec(function(err, doc) {
        ...
        fullUpdateCourse(doc, req, res);
    });
};

function fullUpdateCourse(student, req, res) {
...
module.exports.performPatchUpdate = function(req, res) {
    ...
    Student.findById(req.params.studentId).select("courses").exec(function(err, doc) {
        ...
        patchCource(doc, req, res);
    });
};

function patchCource(student, req, res) {
    ...
    if (req.body.name) { couru.name = req.body.name; }
    if (req.body.description) { couru.description = req.body.description; }
    if (req.body.courseNumber) { couru.courseNumber = req.body.courseNumber; }
